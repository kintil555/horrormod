...
#
#   (2) This script targets any POSIX-compliant shell, so it avoids bashisms.
#
##############################################################################
...
# This is reliable if the script is executed from the project directory
...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # temporary variables.  (12 is enough)
        set -- "$@" "$arg"
        shift
    done
fi
